...
/*11.  Which were the busiest years for 'John Travolta', show the year and the number of movies he made each year for any year 
...
JOIN actor   ON actorid=actor.id
WHERE ord = 1 AND movie.id = ANY (SELECT movie.id FROM movie
                                  JOIN casting ON movie.id=movieid
                                  JOIN actor   ON actorid=actor.id
...
WHERE casting.movieid = ANY(SELECT movieid FROM casting 
